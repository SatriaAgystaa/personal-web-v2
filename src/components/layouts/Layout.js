...
const Layout = ({ children, toggleDarkMode }) => {
  ...
  useEffect(() => {
    const initCanvasAnimations = () => {
      ...
      document.querySelector('.stars-container').appendChild(starsCanvas);
      ...
      document.querySelector('.shooting-stars-container').appendChild(shootingCanvas);
      document.querySelector('.constellation-container').appendChild(constellationCanvas);
      ...
      for (let i = 0; i < starCount; i++) {
        stars.push({
          ...
          rotation: 0,
          rotationSpeed: (0.01 + Math.random() * 0.03) * (Math.random() > 0.5 ? 1 : -1),
          id: i
        });
      }
      ...
      for (let i = 0; i < constellationCount; i++) {
        ...
        const nearbyStars = stars
          ...
          .filter(star => {
            ...
            return distance < Math.min(starsCanvas.width, starsCanvas.height) * 0.25;
          })
          .sort((a, b) => {
        ...
        for (let j = 0; j < Math.min(starCountInConstellation, nearbyStars.length); j++) {
          ...
          if (starIndices.length > 2 && Math.random() > 0.7) {
            ...
            if (randomPair[0] !== randomPair[1]) {
              const exists = connections.some(
                ([a, b]) => (a === randomPair[0] && b === randomPair[1]) || 
      ...
      // Draw star shape
      const drawStar = (ctx, x, y, radius, points, innerRadiusRatio, rotation = 0) => {
        ctx.beginPath();
      ...
      const animate = (timestamp) => {
        ...
        stars.forEach(star => {
          const twinkle = 0.4 + 0.6 * Math.sin(timestamp * 0.0008 * star.speed + star.phase);
          ...
          drawStar(starsCtx, star.x, star.y, star.size * 2, star.points, star.innerRadiusRatio, star.rotation);
          starsCtx.fill();
          ...
          drawStar(starsCtx, star.x, star.y, star.size, star.points, star.innerRadiusRatio, star.rotation);
          starsCtx.fill();
        });

        // Update constellation progress
        constellationProgressRef.current = (constellationProgressRef.current + 0.002) % 100; // Faster progress

        // Draw constellations with sequential line drawing
        constellations.forEach(constellation => {
          const pulseIntensity = 0.5 + 0.5 * Math.sin(timestamp * constellation.pulseSpeed + constellation.pulsePhase);
          
          // Update drawing progress faster
          constellation.drawProgress = Math.min(1, constellation.drawProgress + 0.004 * constellation.drawSpeed);
          
          // Draw connections between stars sequentially
          constellation.connections.forEach(([starA, starB], connectionIndex) => {
            ...
            const connectionStartThreshold = connectionIndex / constellation.connections.length;
            if (constellation.drawProgress < connectionStartThreshold) return;
            
            // Calculate progress for this specific connection (0 to 1)
            const connectionProgress = Math.min(1, 
              (constellation.drawProgress - connectionStartThreshold) * constellation.connections.length);
            
            constellation.connectionProgress[connectionIndex] = 
              Math.min(1, constellation.connectionProgress[connectionIndex] + 0.02); // Faster connection progress
            ...
            if (connectionProgress < 1) {
              const partialX = star1.x + (star2.x - star1.x) * connectionProgress;
              const partialY = star1.y + (star2.y - star1.y) * connectionProgress;
            ...
            if (connectionProgress > 0.3) {
              const dotSize = 1.5 * constellation.connectionProgress[connectionIndex];
              const dotColor = document.documentElement.classList.contains('dark')
              ...
              constellationCtx.beginPath();
              constellationCtx.arc(star1.x, star1.y, dotSize, 0, Math.PI * 2);
              constellationCtx.fill();
              
              if (connectionProgress === 1) {
                constellationCtx.beginPath();
                constellationCtx.arc(star2.x, star2.y, dotSize, 0, Math.PI * 2);
                constellationCtx.fill();
              }
            }
          });
        ...
        shootingStars.forEach(star => {
          ...
          if (star.x > starsCanvas.width + star.tailLength || 
          ...
          const gradient = shootingCtx.createLinearGradient(
            star.x, star.y,
            star.x - star.tailLength, star.y - star.tailLength * 0.3
          );
          ...
          shootingCtx.lineTo(star.x - star.tailLength, star.y - star.tailLength * 0.3);
          shootingCtx.stroke();
          ...
          headGradient.addColorStop(0, `${star.color.replace(')', ', 0.9)')}`);
  ...
  return (
    <div className="relative flex flex-col min-h-screen bg-[#FDFDFD] dark:bg-[#070707] overflow-hidden">
      ...
      {/* Global styles */}
      <style jsx global>{`
        .stars-canvas, .shooting-canvas, .constellation-canvas {
